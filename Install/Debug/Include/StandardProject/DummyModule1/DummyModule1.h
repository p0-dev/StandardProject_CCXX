

/* --------------------------------------------------------------------------------------
** COPY RIGHT / LEFT
** --------------------------------------------------------------------------------------
**  (C) 2024 Hardware And Software Solution (HASS)
**  (C) 2024 Thanh Tran / Nhuan Tran
**  (C) 2024 Khai Tran / Vy Tran
**
**  Confidential Proprietary Information. Distribution Limited.
**
**  This source code is permitted to be used only in projects contracted
**  with HASS and Thanh Tran/Khai Tran/Vy Tran, and any other use is prohibited.
**  If you use it for other purposes or change the source code,
**  you may take legal responsibility and
**
**  FUCK YOU!.
**
**  In this case, there is no warranty and technical support.
-------------------------------------------------------------------------------------- */


// ======================================================================================
// INTENTIONAL LEAVE BLANK
// ======================================================================================


/* --------------------------------------------------------------------------------------
** FILE INFORMATION
** --------------------------------------------------------------------------------------
** NAME             | DummyModule1.h
** --------------------------------------------------------------------------------------
** LOCATION         | Root/Codebase/Include/DummyModule1/DummyModule1.h
** --------------------------------------------------------------------------------------
** DESCRIPTION      | Header file for DummyModule1
-------------------------------------------------------------------------------------- */


#ifndef __DUMMY_MODULE_1_H__
#define __DUMMY_MODULE_1_H__


// ======================================================================================
// DEFINITION - API FUNCTIONS
// ======================================================================================
extern void DummyModule1_Function1 (void);


#endif // __DUMMY_MODULE_1_H__